...
break;case"startsingle":b=flipart.gameOptions;if(b&&!b.absolute){}$("#dGameControls").css({"margin-left":(b.measures.w+10)+"px",height:(b.measures.h)+"px"});
...
flipart.selectedGallery=a;break}}function puzzleSolved(){$("#dSolved").show();flipart.audio.solve();$("#dNext").show().animate({opacity:1},2000)
}function playSingle(b){$("#dSolved").hide();$("#dNext").hide();$("#dLoading").show();document.getElementById("dTiles").innerHTML="";
var a=flipart.urls.newgame,c={};if(b.restart||b.next){a=flipart.urls.nextgame;if(b.restart){c.restart=true}}else{c=flipart.newGameData(b)
}$.ajax(a,{dataType:"json",data:c}).done(function(d){flipart.gameOptions=d.gameOptions;flipart.matrix=d.matrix;flipart.resultMatrix=d.resultMatrix;
flipart.level=d.level;flipart.numTransforms=d.numTransforms}).done(function(){var e=flipart.urls.image+"?t="+new Date().getTime();
...
f<b;f++){e=g.galleries[f];c.append(d({galleryName:e,gallerySrc:flipart.urls.base+"/galleryImage?gallery="+e}))}})}var flipart={gwidth:null,gheight:null,wwidth:null,wheight:null,fwidth:null,fheight:null,urls:{},actions:{playsingle:"playsingle",startnew:"startnew","continue":"continue",instructions:"inst"},gameOptions:null,matrix:null,resultMatrix:null,level:null,numTransforms:null,transformations:[],transIndex:0,difLevel:null,selectedGallery:null,init:function(){this.matrix=null;
this.transformations=[];this.transIndex=0},computeSize:function(f,e){var i=0.95;var b=0.75;var d=this.wwidth=f;var g=this.wheight=e;
b=g/d;if(b<=0.7){b=0.7}else{if(b>=1){b=1}}d*=i;g*=i;var a=g/b;var c=d*b;if(a<d){this.gwidth=a;this.gheight=g}else{this.gwidth=d;
this.gheight=c}this.gwidth=Math.floor(this.gwidth);this.gheight=Math.floor(this.gheight);this.fwidth=this.gwidth-160;this.fheight=this.gheight-30
},newGameData:function(a){var b={maxWidth:flipart.fwidth,maxHeight:flipart.fheight};if(a.difLevel){flipart.difLevel=a.difLevel
}if(flipart.selectedGallery!=null){b.gallery=flipart.selectedGallery}b.difLevel=flipart.difLevel;return b},getRectangle:function(f,b){var e,c,d,a;
if(f.i<b.i){e=f.i;c=b.i}else{e=b.i;c=f.i}if(f.j<b.j){d=f.j;a=b.j}else{d=b.j;a=f.j}return{imin:e,imax:c,jmin:d,jmax:a}},flip:function(c){var b=this.getRectangle(this.mouseMoves.start,this.mouseMoves.end);
...
if(a&&this.transIndex>0){c=this.transformations[this.transIndex-1];this.transIndex--}else{if(!a&&this.transIndex<this.transformations.length){c=this.transformations[this.transIndex];
...
return f}).then(function(f){c.animate({opacity:0},700,function(){c.css({"background-color":"",opacity:""});if(f){puzzleSolved();
f=false}})})}break}},move:function(f){var d,b;var e=[];var c;if(this.state!="click1"){return}this.ismove=true;var a=this._getCoordsForMouse(f);
...
this.flipmusic=a;this.solvedmusic=b;this.backmusic.volume=0.1;this.flipmusic.volume=0.3},startBackmusic:function(){this.backmusic.play()
